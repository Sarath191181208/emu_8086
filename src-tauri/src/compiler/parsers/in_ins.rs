use crate::{
    compiler::{
        ...
        tokens::{registers16bit::Registers16bit, Assembly8086Tokens, registers8bit::Registers8bit},
...
pub(in crate::compiler) fn parse_in(
    ...
    match &high_token.token_type { 
        Assembly8086Tokens::Register16bit(Registers16bit::AX) | Assembly8086Tokens::Register8bit(Registers8bit::AL) => {
            let is_al = match &high_token.token_type{
                Assembly8086Tokens::Register16bit(Registers16bit::AX)  => false,
                _ => true
            };
...
mod tests {
    ...
    test_compile!(out_in_al_and_num, &format!("IN AL, 0x80",), |instructions: &Vec<
        u8,
    >| {
    ...
    test_compile!(out_in_ax_and_num, &format!("IN AX, 0x10",), |instructions: &Vec<
        u8,
    >| {
    ...
    test_compile!(out_in_al_and_dx, &format!("IN AL, DX",), |instructions: &Vec<
        u8,
    >| {
        assert_eq!(instructions, &vec![0xEC])
    });
    
    test_compile!(out_in_ax_and_dx, &format!("IN AX, DX",), |instructions: &Vec<
        u8,
    >| {
